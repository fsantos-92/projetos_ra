...
namespace UnityEngine.UI.Extensions
{
    ...
    public class HorizontalScrollSnap : ScrollSnapBase, IEndDragHandler
    {
        ...
        void Update()
        {
            ...
            if (!_pointerDown)
            {
				if ((_scroll_rect.velocity.x > 0.01 || _scroll_rect.velocity.x < 0.01) && _lerp == false)
                {
        ...
        private void DistributePages()
        {
			...
			_screens = 0; //_screensContainer.childCount;

			for (int i = 0; i<_screensContainer.childCount; i++)
			{
				if (_screensContainer.transform.GetChild(i).gameObject.activeSelf)
					_screens++;
			}
